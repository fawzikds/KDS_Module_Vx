...
#region XML parsing and generation
        ...
        private void ParseRoutingPreferenceManagerFromXML(XElement routingPreferenceManagerXElement)
        {
            ...
            foreach (XNode xRule in routingPreferenceManagerXElement.Nodes())
            {
                if (xRule is XElement)
                {
                    ...
                    ruleIndex = ParseRoutingPreferenceRuleIndexFromXML(xRule as XElement);
                    //routingPreferenceManager.AddRule(groupType, rule);
                    try
                    {
                        routingPreferenceManager.AddRule(groupType, rule, ruleIndex);
                    }
                    catch (System.Exception ruleExcp)
                    {
                        Autodesk.Revit.UI.TaskDialog.Show("RoutingPreferenceBuilder", "I am in ParseRoutingPreferenceManagerFromXML. Exception in AddRule = " + ruleExcp);
                    }
                }
            }
        ...
        /// <summary>
        /// Get RoutingPreferenceRule index from Xml
        /// </summary>
        /// <param name="ruleXElement"></param>
        /// <returns></returns>  

        private int ParseRoutingPreferenceRuleIndexFromXML(XElement ruleXElement)
        {
            XAttribute xaDescription = null;
            XAttribute xaPartName = null;
            ...
            XAttribute xaruleIndex = null;

            int ruleIndex_int = 9;
            int ruleIndex = 10;


            xaruleIndex = ruleXElement.Attribute(XName.Get("ruleIndex"));
            //Autodesk.Revit.UI.TaskDialog.Show("RoutingPreferenceBuilder", "I am in ParseRoutingPreferenceRuleFromXML. xruleIndex = " + xruleIndex.Value);



            bool isParsable = Int32.TryParse(xaruleIndex.Value, out ruleIndex_int);
            if (isParsable)
            {
                ruleIndex = ruleIndex_int;
                Autodesk.Revit.UI.TaskDialog.Show("RoutingPreferenceBuilder", "I am in ParseRoutingPreferenceRuleFromXML. Converted ruleIndex to int = " + xaDescription.Value + " :  " + xaruleIndex.Value);
            }
            else
                Autodesk.Revit.UI.TaskDialog.Show("RoutingPreferenceBuilder", "I am in ParseRoutingPreferenceRuleFromXML. Could NOT convert ruleIndex to int ");

            ruleIndex = ruleIndex_int;
            //Autodesk.Revit.UI.TaskDialog.Show("RoutingPreferenceBuilder", "I am in ParseRoutingPreferenceRuleFromXML. ruleIndex = " + xaruleIndex.Value);
